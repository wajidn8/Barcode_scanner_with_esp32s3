                                 ...
                                 void *arg)
{
    ...
    case HID_HOST_INTERFACE_EVENT_INPUT_REPORT:
        ...
        printf("RAW (%d): ", data_length);
        for (int i = 0; i < data_length; i++) {
            printf("%02X ", data[i]);
        }
        printf("\n");

        // try both normal boot and report-ID based packets
        int key_pos = -1;
        uint8_t modifier = 0;

        if (data_length >= 8) {
            // boot keyboard format
            modifier = data[0];
            key_pos = 2;
        }

        if (data_length >= 9 && data[0] != 0) {
            // report ID packet
            modifier = data[1];
            key_pos = 3;
        }
        ...
        if (key_pos != -1) {
            for (int i = key_pos; i < data_length; i++) {
                uint8_t key_code = data[i];

                if (key_code == 0) continue;

                unsigned char key_char;
                if (hid_keyboard_get_char(modifier, key_code, &key_char)) {
                    if (key_char == '\r') {
                        ...
                        if (barcode_index < sizeof(barcode) - 1) {
                            barcode[barcode_index++] = key_char;
                        }
                    }
                }
            }
